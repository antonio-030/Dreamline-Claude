  ...
  card.innerHTML = `
    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:${masked ? '12px' : '0'};">
      <div style="display:flex; align-items:center; gap:12px;">
        <div style="width:40px; height:40px; border-radius:10px; background:${ok ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)'}; display:flex; align-items:center; justify-content:center; font-size:20px; color:${ok ? 'var(--success)' : 'var(--danger)'};">
          ${ok ? '&#10003;' : '&#10007;'}
        </div>
        <div>
          <div style="font-size:16px; font-weight:600; color:${ok ? 'var(--success)' : 'var(--danger)'};">
            ${ok ? 'Authentifiziert' : 'Nicht authentifiziert'}
          </div>
          <div style="font-size:12px; color:var(--text-muted);">
            ${ok ? (auth.email || auth.subscription || auth.method || '') : 'Claude-Abo wird für Dreams benötigt'}
          </div>
        </div>
      </div>
    ...
    <div style="padding:12px; background:rgba(239,68,68,0.08); border:1px solid rgba(239,68,68,0.2); border-radius:8px; font-size:13px; color:var(--text-muted);">
      Kein Token hinterlegt. Klicke <strong style="color:var(--text);">Anmelden</strong> um einen Token zu erstellen.
    </div>`}`;
}
...
function openAuthModal() {
  ...
  box.innerHTML = `
    <div class="modal-header">
      <h3 class="modal-title">Claude-Abo Anmeldung</h3>
  ...
  setTimeout(() => document.getElementById('auth-token-input')?.focus(), 100);
}
